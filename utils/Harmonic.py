...
def calculate_Ts(D, Sm, Vm, Am, Jm):
    """
    ...
    Ts_d = (abs(D) * pi / (16 * Sm)) ** (1 / 4)
    Ts_v = (Vm * pi / (4 * Sm)) ** (1 / 3)
    Ts_a = sqrt(Am * pi / (2 * Sm))
    Ts_j = pi * Jm / (2 * Sm)
...
def calculate_jerk(t, tk, jerk, J_max):
    """
    ...
    jerk[k] = J_max / 2 * (1 - np.cos(pi * tau))
    k = (t >= tk[12]) & (t < tk[13])
...
def curve(D, Dk, Sm, Vm, Am, Jm, num=1000):
    ...
    for i in range(n):
        ...
        jerk[i] = copysign(1, Dk[i]) * calculate_jerk(t, tk[i], jerk[i], Jm_sync[i])
